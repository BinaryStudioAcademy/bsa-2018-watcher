namespace Watcher.Common.Validators
{
    ...
    public class ChartValidator : AbstractValidator<ChartRequest>
    {
        ...
        {
            RuleFor(x => x.Type).NotEmpty().WithMessage("Please specify a valid Name. Max length: 50, Min length: 3");
            RuleFor(x => x.Source).NotEmpty().Length(2, 50).WithMessage("Please specify a valid Count. Max length: 50, Min length: 2");
            RuleFor(x => x.ShowCommon).NotEmpty().Length(2, 50).WithMessage($"Please specify a valid Show Common. Max length: 50, Min length: 2");
